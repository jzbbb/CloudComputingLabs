...
{
  cout << "coordinator information = " << coordinator_info.ip << ':'
       << coordinator_info.port << endl;
  cout << "participant information = " << participant_info.ip << ':'
       << participant_info.port << endl;
  ...
  int flag = 1;
  if (setsockopt(participant_socket, SOL_SOCKET, SO_REUSEADDR, &flag,
                 sizeof(int)) == -1)
  {
    perror("set coordinator_socket error!");
    exit(1);
  }
  if (bind(participant_socket, (struct sockaddr *)&serv_addr,
           sizeof(serv_addr)) == -1)
  {
...
void participant::start()
{
  ...
  {
    int conn_sock =
        accept(participant_socket, (struct sockaddr *)&serv_addr, &sin_size);
    if (conn_sock < 0)
    {
...
void participant::process(int conn_socket)
{
  ...
  while (1)
  {
    ...
    for (map<string, string>::iterator it = memory_new.begin();
         it != memory_new.end(); it++)
    {
...
void participant::rollback()
{
  commit_ID--;
  memory_new.clear();
  for (map<string, string>::iterator it = memory_old.begin();
       it != memory_old.end(); it++)
  {
