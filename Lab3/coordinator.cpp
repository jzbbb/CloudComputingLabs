  ...
  while (!input.eof())
  {
    ...
    if (ss.rdbuf()->in_avail() != 0)
    {
      ...
      else if (parameterName == "coordinator_info")
      {
        if (ss.rdbuf()->in_avail() == 0)
        {
          perror("coordinator_info is null in the configuration fileï¼?");
          exit(0);
        }
        ss >> value;
        int j = 0;
      ...
      else if (parameterName == "participant_info")
      {
        if (ss.rdbuf()->in_avail() == 0)
        {
          perror("participant_info is null in the configuration fileï¼?");
          exit(0);
        }
        ss >> value;
        int j = 0;
...
void coordinator::init()
{
  ...
  int flag = 1;
  if (setsockopt(coordinator_socket, SOL_SOCKET, SO_REUSEADDR, &flag,
                 sizeof(int)) == -1)
  {
    perror("set coordinator_socket error!");
    exit(1);
  }
  if (bind(coordinator_socket, (struct sockaddr *)&serv_addr,
           sizeof(serv_addr)) == -1)
  {
...
void coordinator::Heartbeat_detection(int mode)
{
  ...
  if (mode == 3 || mode == 5)
  {
    ...
    for (it = failed_participant.begin(); it != failed_participant.end();)
    {
      ...
      if ((connect(clientfd, (struct sockaddr *)&seraddr, sizeof(seraddr))) !=
          0)
      {
...
void coordinator::try_connect(coordinator *cor, int i, int clientfd,
                              struct sockaddr_in seraddr)
{
  int n = 20;
  while (n--)
...
void coordinator::ProcessTimeout()
{
  for (int i = 0; i < participant_number; i++)
  {
    if (participant_work[i] == false &&
        succeed_participant.find(i) != succeed_participant.end())
    {
      succeed_participant.erase(i);
      failed_participant.insert(i);
      close(connect_participant_pos_socket[i]);
...
int coordinator::RecvCommand(int conn_socket)
{
  ...
  cout << "command name:" << command.Command_name << ' ' << command.value1
       << ' ' << command.value2 << endl;
...
void coordinator::start()
{
  ...
  while (1)
  {
    ...
    if (id >= participant_number)
      id -= participant_number;
    int conn_sock =
        accept(coordinator_socket, (struct sockaddr *)&serv_addr, &sin_size);
    if (conn_sock < 0)
    {
    ...
    else if (command.Command_name == "SET")
    {
      ...
      if (recv_message("prepared", n, 500000) == n)
      {
        ...
        {
          ProcessTimeout();
          for (it = succeed_participant.begin();
               it != succeed_participant.end(); it++)
          {
            int i = *it;
...
string coordinator::get_message(int i)
{
  ...
  while (t--)
  {
    ...
    {
      memset(buf, 0, 255);
      if (events[i].data.fd == connect_participant_pos_socket[i])
        t = 0;
      int len;
  ...
  while (t--)
  {
    ...
    for (int i = 0; i < num; i++)
    {
      ...
      if (read(events[i].data.fd, buf, s.length() + 5) > 0)
      {
        ...
        {
          res++;
          participant_work[connect_participant_socket_pos[events[i].data.fd]] =
              true;
        }
        else
        {
  ...
  while (t--)
  {
    ...
    for (int i = 0; i < num; i++)
    {
      ...
      if (read(events[i].data.fd, buf, 255) > 0)
      {
        cout << "receve message from participant:" << buf << endl;
        int k = atoi(buf);
        participants_commit_ID
            [connect_participant_socket_pos[events[i].data.fd]] = k;
        res++;
        participant_work[connect_participant_socket_pos[events[i].data.fd]] =
            true;
      ...
      else
      {
        ...
        failed_participant.insert(k);
        participants_commit_ID
            [connect_participant_socket_pos[events[i].data.fd]] = -1;
        close(events[i].data.fd);
      }
    }
    if (num == n)
...
void coordinator::recovery()
{
  while (1)
  {
    if (participants_commit_ID.size() == 0)
      return;
    int maxID = -1;
    ...
    while (1)
    {
      ...
      if (s == "")
      {
        if (succeed_participant.find(good.front()) !=
            succeed_participant.end())
        {
